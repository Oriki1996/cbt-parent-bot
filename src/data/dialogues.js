...
export const structuredDialogues = {
  'listening_challenge': {
    ...
    dialogueSteps: [
      {
        botMessage: (childName) => `אני מבין/ה שאת/ה חווה קושי כש${childName} לא מקשיב/ה. זה יכול להיות מאוד מתסכל. האם תוכל לספר לי מתי זה קורה בדרך כלל?`,
        inputType: 'rubric',
        rubricOptions: [
      ...
      {
        botMessage: (childName) => `כשאת/ה מבקש/ת משהו מ${childName} והוא/היא לא מגיב/ה, מה בדרך כלל המחשבה הראשונה שעוברת לך בראש?`,
        inputType: 'text',
        nextStepLogic: { 'default': { nextStep: 5 } }
      },
      {
        botMessage: (childName) => `כשאת/ה מנסה לדבר עם ${childName} והוא/היא לא מקשיב/ה, איך זה מרגיש לך?`,
        inputType: 'rubric',
        rubricOptions: [
      ...
      {
        botMessage: (childName) => `שימוש במסכים יכול להפוך מתבגרים לפחות קשובים לסביבה. בואו נחשוב על כמה אסטרטגיות למצב הזה. איזו מהאפשרויות הבאות נראית לך הכי מעשית?`,
        inputType: 'rubric',
        rubricOptions: [
      ...
      {
        botMessage: (childName) => `כש${childName} "תמיד" לא מקשיב/ה, יש לי שאלה חשובה: האם באמת תמיד? האם אין אפילו רגע אחד ביום שהוא/היא מקשיב/ה או מגיב/ה?`,
        inputType: 'text',
        nextStepLogic: { 'default': { nextStep: 14 } }
      },
      {
        botMessage: () => `אני מבין/ה את התסכול שלך. המחשבה הזו שעברה בראשך היא מה שאנחנו קוראים לה "מחשבה אוטומטית". בואו נבחן יחד - איזה מהמשפטים הבאים מתאר הכי טוב את המחשבה הזו?`,
        inputType: 'rubric',
        rubricOptions: [
      ...
      // המשך הדיאלוג...
      {
        botMessage: () => `תחושות התסכול והחוסר הבנה הן טבעיות לחלוטין. המטרה שלנו היא לא לחסל אותן, אלא למצוא דרכים יותר יעילות לתקשר. בואו נתחיל עם משהו פשוט - האם תוכל/י לחשוב על דרך שונה לגשת ל${childName} שעוד לא ניסית?`,
        inputType: 'text',
  ...
  'communication_skills': {
    ...
    dialogueSteps: [
      {
        botMessage: (childName) => `אני רוצה להציג בפניך טכניקת תקשורת שיכולה לעזור בשיחות עם ${childName}, במיוחד ברגעים של מתח. הטכניקה נקראת 'הקשבה משקפת', והיא יכולה לסייע כשהתקשורת נתקעת. האם זה נשמע שימושי?`,
        inputType: 'rubric',
        rubricOptions: [
      ...
      {
        botMessage: () => `מעולה! הקשבה משקפת כוללת שלושה שלבים פשוטים:
...
האם תרצה/י לתרגל את זה יחד?`,
      ...
      {
        botMessage: () => `אני מבין/ה את הספקנות. רבים מההורים שאני עובד/ת איתם מרגישים כך בהתחלה. הקשבה משקפת אינה פתרון קסם, אבל מחקרים מראים שהיא באמת משפרת תקשורת.
...
האם תהיה/י מוכן/ה לשמוע יותר על איך זה עובד?`,
  ...
  'resistance_management': {
    ...
    dialogueSteps: [
      {
        botMessage: () => `נראה שאת/ה חווה התנגדות משמעותית מהמתבגר/ת שלך. זה יכול להיות מאוד מתסכל. אני רוצה להבין טוב יותר - איזה סוג של התנגדות אתה נתקל/ת בו?`,
        inputType: 'rubric',
        rubricOptions: [
